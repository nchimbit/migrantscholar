...
async function getTrendingTopics() {
  ...
  const response = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Today is ${today}. Generate 5 blog post topics for a scholarship website targeting migrant students, refugees and asylum seekers. Mix countries: UK, USA, Germany, Canada, Turkey, Australia. At least 2 must be fully funded awards. Vary the types.
...
cat > scripts/generate-posts.mjs << 'EOF'
import Anthropic from "@anthropic-ai/sdk";
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
...
async function generatePost(topic) {
  ...
  const response = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Write a complete SEO-optimised blog post for MigrantScholar.com.
...
Respond ONLY with valid JSON, no markdown, no backticks:
{"title":"...","metaDescription":"...","excerpt":"...","content":"...","tags":[],"readingTime":6}`
    }]
  });
  const raw = response.content[0].text.trim().replace(/^```json\n?/,"").replace(/\n?```$/,"");
...
async function main() {
  console.log("🚀 MigrantScholar daily post generation starting...\n");
  try {
