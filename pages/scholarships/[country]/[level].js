...
const levelData = {
  masters:{label:"Master's",desc:"postgraduate Master's degree",keywords:["master","postgraduate","msc","graduate"]},
  phd:{label:"PhD",desc:"doctoral research",keywords:["phd","doctoral","doctorate","research"]},
  undergraduate:{label:"Undergraduate",desc:"Bachelor's degree",keywords:["undergraduate","bachelor","degree"]},
  postdoctoral:{label:"Postdoctoral",desc:"postdoctoral research",keywords:["postdoc","fellowship","research grant"]},
};

export default function CountryLevelPage({ country, level, posts, cData, lData }) {
  return (
    <>
      <Head>
        <title>{lData.label} Scholarships in {cData.fullName} for Migrants 2026 — MigrantScholar</title>
        <meta name="description" content={`Verified ${lData.label} scholarships in ${cData.fullName} for migrants, refugees and asylum seekers. ${cData.topScholarship} and more. Updated daily.`} />
        <link rel="canonical" href={`https://migrantscholar.vercel.app/scholarships/${country}/${level}`} />
      </Head>
      <Navbar />
      <div style={{background:"#E6F4F1",padding:"1.5rem 2rem 0"}}>
        <div style={{maxWidth:"1080px",margin:"0 auto"}}>
          <div style={{background:"linear-gradient(135deg,#0D6E6E 0%,#0A5555 100%)",borderRadius:"14px",padding:"2rem"}}>
            ...
            <h1 style={{fontSize:"1.5rem",fontWeight:900,color:"#fff",marginBottom:".5rem",lineHeight:1.3}}>{lData.label} Scholarships in {cData.fullName} for Migrants & Refugees 2026</h1>
            <p style={{fontSize:"13px",color:"rgba(255,255,255,.8)",maxWidth:"560px",lineHeight:1.6}}>Verified {lData.desc} scholarships in {cData.fullName} for migrants, refugees and asylum seekers. {cData.topScholarship} and more.</p>
            <div style={{display:"flex",gap:".75rem",marginTop:"1rem",flexWrap:"wrap"}}>
              ...
              <Link href={`/by-level/${level}`} style={{background:"rgba(255,255,255,.15)",color:"#fff",padding:"6px 14px",borderRadius:"6px",fontSize:"12px",fontWeight:600,textDecoration:"none"}}>All {lData.label} Scholarships →</Link>
            </div>
          </div>
        </div>
      </div>
      <div style={{maxWidth:"1080px",margin:"0 auto",padding:"2rem"}}>
        <div style={{fontSize:"12px",color:"#6b7280",marginBottom:"1.5rem"}}>
          <Link href="/" style={{color:"#0D6E6E",textDecoration:"none"}}>Home</Link> › <Link href={`/countries/${countryMap[country]||country}`} style={{color:"#0D6E6E",textDecoration:"none"}}>{cData.fullName}</Link> › {lData.label} Scholarships
        </div>
        <div style={{display:"flex",gap:".5rem",flexWrap:"wrap",marginBottom:"1.5rem"}}>
          {["refugees","asylum-seekers","without-ielts","migrants"].map(e=>(
            <Link key={e} href={`/scholarships/${country}/${e}`} style={{fontSize:"11px",color:"#0D6E6E",background:"#E6F4F1",border:"1px solid #A7D4CC",borderRadius:"20px",padding:"4px 12px",textDecoration:"none",fontWeight:600}}>{e.replace(/-/g," ")} →</Link>
          ))}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:"1rem",marginBottom:"2rem"}}>
          {posts.length > 0 ? posts.map(post=>(
            ...
            <div style={{gridColumn:"1/-1",textAlign:"center",padding:"3rem"}}>
              <p style={{color:"#6b7280",marginBottom:"1rem"}}>More {lData.label} guides for {cData.fullName} coming soon.</p>
        ...
        <div style={{background:"#F5A623",borderRadius:"12px",padding:"2rem",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"1.5rem",flexWrap:"wrap"}}>
          <div>
            ...
            <p style={{fontSize:"13px",color:"rgba(10,42,42,.7)"}}>Free alerts for new {lData.label} scholarships in {cData.fullName}.</p>
          </div>
...
export async function getStaticPaths() {
  ...
  const levels = ["masters","phd","undergraduate","postdoctoral"];
  const paths = [];
  for (const country of countries) {
    for (const level of levels) {
      paths.push({ params: { country, level } });
    }
  }
...
export async function getStaticProps({ params }) {
  ...
  const lData = levelData[level] || levelData.masters;
  const posts = allPosts.filter(p => {
    const text = (p.title + " " + p.excerpt).toLowerCase();
    const matchCountry = p.country === cName || text.includes(cName.toLowerCase());
    const matchLevel = lData.keywords.some(k => text.includes(k));
    return matchCountry && matchLevel;
  }).slice(0, 12);
  return { props: { country, level, posts, cData, lData }, revalidate: 3600 };
}
